      ...
      if len(locations) >= 3:
        ...
        if not os.path.exists(state_file):
          with open(state_file, mode='w', newline='') as file:
            ...
            writer.writerow(['Title', 'Price', 'Location', 'URL', 'Image URL'])
        ...
        for sneaker in sneakers:
          ...
          print(f"{Fore.YELLOW}Current location: {city}, {state}{Fore.RESET}\n")

          print(f"{Fore.YELLOW}Current sneaker: {sneaker}{Fore.RESET}\n")
          ...
          print(f"{Fore.GREEN}Status code response: {response.status_code}{Fore.RESET}\n")

          if response.status_code == 200:
            data = response.json()
          else:
            print(f"{Fore.RED}Failed to fetch data:{Fore.RESET}\n", response.status_code)
          ...
          with open(state_file, mode='a', newline='') as file:
            ...
            for i in data:
              if 'listing' in i:
                ...
                writer.writerow([title, price, location, url, imageUrl])

          print(f"{Fore.YELLOW}Number of listings scraped: {listing_count}{Fore.RESET}\n")

          end_time = time.time()
          print(f"{Fore.CYAN}Completion time: {round(end_time - start_time, 2)} seconds{Fore.RESET}\n")

          print("-" * 20)

  overall_end_time = time.time()
  print(f"{Fore.CYAN}Scraping ended at {round(overall_end_time - overall_start_time, 2)} seconds{Fore.RESET}\n")
